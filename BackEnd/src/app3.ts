...
client.on('message', async (message: Message) => {
    ...
    const contactId = isFromMe ? message.to.replace(/[^0-9]/g, '') : message.from.replace(/[^0-9]/g, '');
    ...
    const sender = isFromMe ? 'Você' : 'Contato';
    const textContent = `${new Date(message.timestamp * 1000).toLocaleString()} (${sender}): ${message.body}\n`;
